...
def calculate_total_price(items: Iterable[Item], discount: Decimal = Decimal(0.0)) -> Decimal:
    total_price = Decimal(sum(item.price for item in items) * (Decimal(1.0)-discount))
    return total_price
...
def process_order(order: Order) -> None:
    ...
    if order.type is OrderType.ONLINE:
