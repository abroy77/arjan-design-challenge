...
PaymentInterface = Callable[[Decimal], None]
...
def cc_payment(total: Decimal) -> None:
    ...
    print(
        ...
        f" with card number {card_number_masked} and"
        f" expiration date {expiration_date} and CCV {ccv_masked}..."
    )


def paypal_payment(total: Decimal) -> None:
...
def apple_pay_payment(total: Decimal) -> None:
...
PAYMENT_TYPES: Dict[str, PaymentInterface] = {
    "cc": cc_payment,
    "paypal": paypal_payment,
    "apple pay": apple_pay_payment
}
...
class ShoppingCart:
    ...
    def handle_payment(self) -> None:
        ...
        payment_type: str = input("what payment system would you like"
                                  "cc/paypal/apple pay: ")

        if payment_type in PAYMENT_TYPES:
            PAYMENT_TYPES[payment_type](self.total)
        else:
