...
from db import EventCreate, TicketCreate, UpdateTicket
import operations
import uvicorn
...
async def cancel_ticket(ticket_id: int, database: Session = Depends(get_db)):
    try:
        ticket = operations.delete_ticket(ticket_id=ticket_id, database=database)
        return ticket
    except Exception as _:
        raise HTTPException(status_code=500)

@app.put("/ticket/{ticket_id}")
async def update_ticket(ticket_id: int, update_ticket: UpdateTicket, database: Session = Depends(get_db)):
    try:
        ticket = operations.update_ticket(ticket_id=ticket_id, update_ticket=update_ticket, database=database)
        return ticket
    except Exception as _:
        raise HTTPException(status_code=500)
