...
from payment_handlers import all_payments, get_handler, handler_exists
...
def handle_payment(total: Decimal) -> None:
    payment_type = input(
        f"What payment method would you like to use? ({all_payments()}) "
    )

    if handler_exists(payment_type):
        # Process the payment
        handler = get_handler(payment_type)
        handler(total)
    else:
